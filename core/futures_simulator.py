...
class FuturesBacktester:
    ...
    def load_data(self, days_limit: int = None) -> Optional[pd.DataFrame]:
        ...
        # Parsing robusto del símbolo
        symbol_clean = self.symbol.replace('USDT', '').replace('_4h', '').replace('_1h', '').replace('_1d', '').upper()
        
        # Probar ambas estructuras de nombres de archivo
        possible_filenames = [
            f"{symbol_clean}USDT_{self.timeframe}.json",      # Estructura nueva: XRPUSDT_4h.json  
            f"{symbol_clean}_USDT_{self.timeframe}.json"      # Estructura vieja: BTC_USDT_4h.json
        ]
        
        cache_path = None
        for filename in possible_filenames:
            test_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'cache_real', filename)
            if os.path.exists(test_path):
                cache_path = test_path
                break
        
        if cache_path is None:
            print(f"❌ No se encontró archivo para {symbol_clean}USDT en timeframe {self.timeframe}")
            print(f"   Intenté: {possible_filenames}")
            return None
...
def main():
    ...
    parser.add_argument('symbol', help='Símbolo a analizar (ej: XRPUSDT_1d, BTCUSDT, DOT_4h)')
    parser.add_argument('--balance', type=float, default=FUTURES_CONFIG['initial_balance'], help='Balance inicial')
    parser.add_argument('--leverage', type=int, default=FUTURES_CONFIG['leverage'], help='Apalancamiento')
    parser.add_argument('--position_size', type=float, default=FUTURES_CONFIG['position_size_usd'], help='Tamaño de posición en USD')
    parser.add_argument('--days', type=int, default=None, help='Limitar a últimos N días')
    
    args = parser.parse_args()
    
    # Parsing escalable del símbolo y timeframe
    def parse_symbol_timeframe(input_symbol):
        """Extraer símbolo y timeframe de cualquier formato."""
        # Timeframes válidos
        valid_timeframes = ['1h', '4h', '1d']
        
        # Buscar timeframe al final del string
        for tf in valid_timeframes:
            if input_symbol.endswith(f'_{tf}'):
                return input_symbol[:-len(f'_{tf}')], tf
            elif input_symbol.endswith(tf):
                return input_symbol[:-len(tf)], tf
        
        # Si no encuentra timeframe, usar 4h por defecto
        return input_symbol, '4h'
    
    symbol, timeframe = parse_symbol_timeframe(args.symbol)
